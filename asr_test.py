...
if __name__ == '__main__':
    ...
    # model = load_model('capp_asr_model_weights.h5')  # 加载自己训练模型
    wavs = []
    wavs.append(get_wav_mfcc("./wav/seven/0b77ee66_nohash_2.wav"))
    X = np.array(wavs)
    print(X.shape)
    result = model.predict(X[0:1])[0]  # 识别出第一个音频的结果，多张图的时候，把后面的[0] 去掉，返回的就是多个音频的结果
    print("识别结果", result)
